...
object Interaction2 extends Interaction2Interface:
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] =
    // Keep signal of year slider value up-to-date
    Signal(
      // If in range, take sliders value
      if selectedLayer().bounds.contains(sliderValue()) then
        sliderValue()
      // If less than range take minimum value
      else if selectedLayer().bounds.head>sliderValue() then
        selectedLayer().bounds.head
      // Else must be greater and take end of range
      else
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] =
    Signal("target/"+selectedLayer().layerName.id.toLowerCase+"/"+selectedYear().toString)
  ...
  def caption(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] =
    Signal(selectedLayer().layerName.id+" ("+selectedYear().toString+")")
