...
case class WikipediaArticle(title: String, text: String):
  /**
    ...
    */
  // def contains(lang: String): Boolean = title.contains(lang) || text.contains(lang)
...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val wikiRdd: RDD[WikipediaArticle] = 
    sc
      ...
      .map(WikipediaData.parse(_))

  // Get RDD with all articles that mention Specific Language
  def articlesWithLang(lang: String, rdd: RDD[WikipediaArticle]): RDD[WikipediaArticle]= 
    rdd
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd
      .flatMap(article => 
        langs
          .filter(article.mentionsLanguage(_))
          .map((_, article)))
      .groupByKey
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = 
    index
      .map((str, list) => (str, list.count(article => true)))
      .sortBy(-_._2)
      .collect()
      .toList
