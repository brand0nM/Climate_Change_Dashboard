...
trait Solver extends GameDef:
   ...
   */
  def done(b: Block): Boolean = b.isStanding && b._1==goal
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.map(block => (block._1, block._2 :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter((b, ls) => !explored.contains(b))

  /**
   * The function `from` returns the lLazy list of all possible paths
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = initial match {
             case (b, ls) #:: lazyTail => {
              val newls = newNeighborsOnly(neighborsWithHistory(b, ls), explored + b)
              newls ++ from(lazyTail ++ newls, explored + b)
            }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = 
    pathsFromStart.filter((b, ls) => done(b))
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case head #:: _ => print(goal);head._2.reverse
    case _ => Nil
  }