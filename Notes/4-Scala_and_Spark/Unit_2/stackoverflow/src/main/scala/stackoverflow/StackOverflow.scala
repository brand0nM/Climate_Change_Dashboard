...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    // grouped
    //   // Create one colelction of questions and answers
    //   .flatMap(_._2)
    //   .groupByKey()
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    def firstLangInTag(tag: String): Option[Int] =
      val index = langs.indexOf(tag); if (index < 0) None else Some(index*langSpread)

    scored
      .map((post, score) =>
        (post
          .tags
          .map(str => firstLangInTag(str).get).get,
        score))
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    // and a map that associate the old `means` values to their new values
    // val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] =
    //   vectors
    //     // for each point in in our collection find closest mean
    //     .map(point => (findClosest(point, means), point))
    //     // collect the set of points in each mean group
    //     .groupByKey
    //     // compute each mean groups new mean
    //     .mapValues(averageVectors)
    //     .collect()
    //     .toMap
    // val newMeans: Array[(Int, Int)] = newMeansMap.values.toArray
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langLabel: String   = langs(grouped.maxBy(_._2)._1)
      val clusterSize: Int    = vs.size
      // percent of the questions in the most common language
      val langPercent: Double = grouped(grouped.maxBy(_._2)._1)*100.0 / clusterSize
      val sorted = vs.map(_._2).toList.sorted
      val medianScore: Int    = 
        if (clusterSize%2==0) ((sorted(clusterSize/2 -1) + sorted(clusterSize/2))/2.0).toInt
